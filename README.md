...
9. 문제가 생기면 `?debug=1`로 접속한 뒤 `게임 방법 > 버그 리포트 복사`를 눌러 내용을 공유한다.
...
일반 접속에서는 상태 패널, 버그 리포트 복사 영역, 친구 테스트 체크리스트가 보이지 않습니다.

## 디버그 모드

일반 접속에서는 오른쪽 아래 상태 패널, 버그 리포트 복사 영역, 친구 테스트 체크리스트가 숨겨진다.
...
디버그 모드에서는 게임 방법 화면에 버그 리포트 복사 영역과 친구 테스트 체크리스트도 함께 표시된다.
...
- 대기방에서 각 참가자는 준비 완료를 눌러야 한다.
- 모든 참가자가 준비 완료 상태가 되면 방장이 게임을 시작할 수 있다.
- 방장이 카테고리, 난이도, 문제 개수, 제한시간을 변경하면 준비 상태가 초기화된다.
- 다시하기 후에도 모든 참가자의 준비 상태가 초기화된다.

## 방장 관리 기능

- 방장은 대기방에서 참가자를 내보낼 수 있다.
- 방장은 자기 자신을 내보낼 수 없다.
- 참가자가 내보내지면 멀티플레이 메뉴로 돌아간다.
- 방장은 대기방에서 방을 종료할 수 있다.
- 방이 종료되면 모든 참가자가 방에서 나가게 된다.
- 참가자 내보내기는 현재 대기방에서만 지원한다.

## 멀티플레이 접속 상태

- 플레이어가 방에 들어오면 online 상태로 표시된다.
- 브라우저를 닫거나 연결이 끊기면 offline 상태로 바뀐다.
- offline 플레이어는 Ready 판정에서 제외된다.
- 대기방에서 2분 이상 offline인 플레이어는 정리될 수 있다.
- 방장이 offline 상태로 오래 유지되면 남은 online 플레이어에게 방장이 넘어간다.
- 같은 브라우저/계정으로 다시 접속하면 기존 playerId로 복귀할 수 있다.
...
친구 테스트 후 Firebase Console > Realtime Database > `feedbacks` 경로에서 제출된 피드백을 확인한다.

- `answer_request`: `submittedAnswer`가 대표 정답과 같은 의미인지 확인하고, 맞다고 판단되면 `quizData`의 해당 문제 `answers` 배열에 추가 후보로 검토한다.
- `question_report`: `emoji`, `hint`, `difficulty`가 애매하거나 잘못되었는지 확인하고 수정 후보로 검토한다.
- 반영 기준: 여러 사용자가 비슷하게 제보했거나, 기존 정답/힌트만으로도 충분히 오해가 생길 수 있을 때 우선 반영한다.
- 반영 후에는 싱글 플레이와 멀티플레이에서 해당 문제가 정상 출제되는지 다시 확인한다.
...
7. 문제가 있으면 `?debug=1`로 접속한 뒤 게임 방법 화면의 `버그 리포트 복사` 버튼을 누르고 내용을 공유한다.
...
8. `?debug=1`로 접속한 뒤 버그 리포트 복사 버튼으로 내용을 공유한다.
