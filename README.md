...
- 방장이 게임을 시작하면 첫 문제가 바로 표시된다.
- 정답 또는 시간초과 후 잠시 뒤 자동으로 다음 문제로 넘어간다.
- 화면에는 자동 이동 안내 문구를 표시하지 않는다.
- 다음 문제는 별도 카운트다운 없이 바로 시작된다.
- 남은 시간 5초가 되면 힌트가 자동 공개된다.
- 힌트 전 정답은 +10점, 힌트 후 정답은 +5점이다.
- 오답은 실시간 오답 로그에 표시된다.

## 멀티플레이 다시하기

- 결과 화면에서 방장은 `같은 방으로 다시하기`를 누를 수 있다.
- 같은 방 코드와 참가자 목록을 유지한 채 대기방으로 돌아간다.
- 모든 플레이어 점수는 0점으로 초기화된다.
- 방장은 카테고리, 난이도, 문제 개수, 제한시간을 다시 조정하고 시작할 수 있다.
- 비방장은 방장이 다시 시작할 때까지 대기한다.
...
- 방장만 카테고리/난이도 변경 가능
- 방장만 게임 시작 가능
- 방장이 게임 시작 클릭 시 첫 문제 바로 표시
- 시작 전 3초 카운트다운 없음
- 모든 참가자에게 같은 문제 표시
- 먼저 맞힌 사람만 점수 획득
- 제한시간 5초 이하에서 힌트 자동 공개
- 시간 초과 후 정답 공개
- 정답 또는 시간초과 후 자동 이동 안내 문구가 표시되지 않음
- 정답 또는 시간초과 후 잠시 뒤 자동 이동
- 다음 문제는 카운트다운 없이 바로 시작
- 방장만 다음 문제 가능
- 최종 순위 표시
- 결과 공유
